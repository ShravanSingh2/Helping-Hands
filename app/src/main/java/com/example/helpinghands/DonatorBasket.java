...
public class DonatorBasket extends AppCompatActivity implements BasketItemsRecycleView2.ItemClickListener{

    String responseData,userID,name;
    int index;
    String itemID;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.donater_basket);
        ...
        numberDisplay = findViewById(R.id.textView22);
    }
    ...
    public void getUserBasket(){
        processJSON(webRequest.sendRequest("https://lamp.ms.wits.ac.za/home/s2173638/get_user_basket.php?BasketID="+ userID ));
    }
    ...
    public void doAddItem(View view) {
        if(networkConnection.isNetworkAvailable()) {
            Intent intent = new Intent(this, AddDonaterItem.class);
            intent.putExtra("userID", userID);
            startActivity(intent);
        }else{
    ...
    public void updateQuantity() {
        for (int i = 0; i < quantityArray.size(); i++) {
            int sum = Integer.parseInt(quantityArray.get(i));
            itemID = itemIDArray.get(i);
            webRequest.sendRequest("https://lamp.ms.wits.ac.za/home/s2173638/updateQuantity.php?u=" + userID + "&i=" + itemID + "&q=" + sum);
        }
    }
    ...
    public void doSave(View view) {
        if(networkConnection.isNetworkAvailable()) {
            updateQuantity();
            Toast.makeText(this, "Items updated succesfully", Toast.LENGTH_SHORT).show();
        }else{
            Toast.makeText(this, "Network unavailable, please try again later", Toast.LENGTH_SHORT).show();
        }
    }
